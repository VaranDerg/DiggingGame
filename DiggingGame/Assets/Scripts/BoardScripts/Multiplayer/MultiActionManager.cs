...
public class MultiActionManager : MonoBehaviour
{
    ...
    public void StartMove(int player)
    {
        ...
        {
            if (pawn.GetComponent<MultiPlayerPawn>().PawnPlayer == player)
            {
                pawn.GetComponent<MultiPlayerPawn>().IsMoving = true;
    ...
    public void StartBuild(int player, string building)
    {
        foreach (GameObject pawn in GameObject.FindGameObjectsWithTag("Pawn"))
        {
            if (pawn.GetComponent<MultiPlayerPawn>().PawnPlayer == player)
            {
                pawn.GetComponent<MultiPlayerPawn>().IsBuilding = true;
                pawn.GetComponent<MultiPlayerPawn>().BuildingToBuild = building;
                pawn.GetComponent<Animator>().Play("TempPawnBlink");
            }
        }
    ...
    public void StartDig(int player)
    {
        foreach (GameObject pawn in GameObject.FindGameObjectsWithTag("Pawn"))
        {
            if (pawn.GetComponent<MultiPlayerPawn>().PawnPlayer == player)
            {
                pawn.GetComponent<MultiPlayerPawn>().IsDigging = true;
                pawn.GetComponent<Animator>().Play("TempPawnBlink");
            }
        }
    ...
    public void StopPawnActions(int player)
    {
        foreach(GameObject pawn in GameObject.FindGameObjectsWithTag("Pawn"))
        {
            if(pawn.GetComponent<MultiPlayerPawn>().PawnPlayer == player)
            {
                pawn.GetComponent<MultiPlayerPawn>().IsMoving = false;
                pawn.GetComponent<MultiPlayerPawn>().IsBuilding = false;
                pawn.GetComponent<MultiPlayerPawn>().IsDigging = false;
